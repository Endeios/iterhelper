...
public class MapTestNeg {
	...
	private static Map<String,Object> myMap2(){
		return new HashMap<String,Object>(){{
			put("key1",NAME+" alternate");
			put("key2",VALUE);
			put("key3",DATE);
		}};
	}
	...
	public void small_equals_lists_are_equals(){
		...
		collectionB.add(new ClassWithMap(NAME, VALUE, DATE, myMap2()));
		
		System.out.println(collectionA);
		System.out.println(collectionB);
		
		Assert.assertFalse(Iterations.isEqual(collectionA, collectionB), "Collections should be Different");
	}
	...
	public void big_equals_sets_are_equals(){
		...
		for(int i =0;i<1000;i++){
			...
			collectionB.add(new ClassWithMap(name+1,(long) i, date,myMap2()));
		}
	...
	public void big_different_sets_are_diferent(){
		...
		for(int i =0;i<1000;i++){
			Date date = new Date();
			collectionA.add(new ClassWithMap(name+1,(long) i, date,myMap()));
			collectionB.add(new ClassWithMap(name+2,(long) i+1, date,myMap2()));
		}
	...
	public void big_different_sets_are_diferent_for_lenght(){
		...
		collectionB.add(new ClassWithMap(name+2,(long) 10000, DATE,myMap2()));
